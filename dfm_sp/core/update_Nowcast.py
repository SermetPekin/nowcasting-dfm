...
def update_nowcast(
    ...
    if isinstance(vintage_old, str):
        vintage_old = pd.to_datetime(vintage_old)
    if isinstance(vintage_new, str):
        vintage_new = pd.to_datetime(vintage_new)
    ...
    # Use pandas vector math to calculate future offsets instead of ordinals
    last_date = pd.to_datetime(Time[-1])
    future = np.array(
        [(last_date + pd.DateOffset(months=i)).to_numpy() for i in range(1, 13)]
    )
    ...
    if freq == "m":
        ...
        m = int(m)
        d = 1
        t_nowcast = np.where(np.datetime64(f"{y:04d}-{m:02d}-{d:02d}") == Time)[0]
    elif freq == "q":
    ...
    # Enforce Numpy integer scalar directly to avoid shape array crashes passing further down
    t_nowcast = t_nowcast[0] if getattr(t_nowcast, "size", 0) > 0 else t_nowcast

    if getattr(t_nowcast, "size", 0) == 0 and not isinstance(
        t_nowcast, (int, np.integer)
    ):
        raise ValueError(f"Period '{period}' is out of nowcasting horizon bounds.")
    ...
    print(
        "\n Nowcast Update: {}".format(pd.to_datetime(vintage_new).strftime("%Y-%m-%d"))
    )
    print(
        "\n Nowcast for: {} ({}), {}".format(
            Spec.SeriesName[i_series][0],
            Spec.UnitsTransformed[i_series][0],
            pd.to_datetime(Time[t_nowcast]).to_period("Q"),
        )
    )
    # Only display table output if a forecast is made
    if forecast is None or np.shape(forecast)[0] == 0:
        print("\n No forecast was made \n")

        # Return empty data safely
        news_table = pd.DataFrame(columns=["Forecast", "Actual", "Weight", "Impact"])
        N_cols = np.shape(X_new)[1]
        data_released = np.zeros(N_cols, dtype=bool)
        return y_old[0], y_new[0], news_table, data_released
    else:
        ...
        print(
            "              {} nowcast:              {:.5f}".format(
                pd.to_datetime(vintage_old).strftime("%Y-%m-%d"), y_old[0]
            )
        )
        print(
            "      Impact from data revisions:      {:.5f}".format(impact_revisions[0])
        )
        print(
            "       Impact from data releases:      {:.5f}".format(
                np.nansum(news_table.Impact)
            )
        )
        print("                                     +_________")
        print(
            "                    Total impact:      {:.5f}".format(
                impact_revisions[0] + np.nansum(news_table.Impact)
            )
        )
        print(
            "              {} nowcast:              {:.5f}".format(
                pd.to_datetime(vintage_new).strftime("%Y-%m-%d"), y_new[0]
            )
        )
...
def News_DFM(X_old, X_new, Res, t_fcst, v_news):
    ...
    if ~np.isnan(X_new[t_fcst, v_news])[0]:
        ...
        for i in range(v_news.shape[0]):  # Loop for each target variable
            ...
            singlenews[:, v_news[i]] = (
                X_new[t_fcst, v_news[i]] - Res_old["X_sm"][t_fcst, v_news[i]]
            )
        ...
        if v_miss.shape[0] == 0:
            ...
            for i in range(n_news):  # Cycle through total number of updates
                ...
                max_val = np.maximum(t_miss[i], t_fcst)
                m = int(max_val[0]) if getattr(max_val, "ndim", 0) > 0 else int(max_val)
            ...
            for i in range(t_miss.shape[0]):
                ...
                X_new_norm = (X_new[t_miss[i], v_miss[i]] - Mx[v_miss[i]]) / Wx[
                    v_miss[i]
                ]
                X_sm_norm = (
                    Res_old["X_sm"][t_miss[i], v_miss[i]] - Mx[v_miss[i]]
                ) / Wx[v_miss[i]]
            ...
            for i in range(v_news.shape[0]):  # loop on v_news
                ...
                if i == 0:
                    totnews = np.matmul(
                        np.matmul(
                            np.matmul(
                                np.matmul(Wx[[v_news[i]]], C[[v_news[i]]][:, :r]), P1
                            ),
                            np.linalg.inv(P2),
                        ),
                        innov.T,
            ...
            singlenews[:], actual[:], forecast[:], weight[:] = (
                np.nan,
                np.nan,
                np.nan,
                np.nan,
            ...
            for i in range(innov.shape[1]):
                ...
                for j in range(v_news.shape[0]):
                    singlenews[j, t_miss[i] - min(t_miss), v_miss[i]] = temp[
                        j, 0, i
                    ].copy()
...
def para_const(X, P, lag, use_numba=True):
    ...
    Sf = SKF(Y, A, C, Q, R, Z_0, V_0, use_numba=use_numba)  # Kalman filter
    ...
    X_sm = np.tile(Wx, (T, 1)) * x_sm + np.tile(
        Mx, (T, 1)
