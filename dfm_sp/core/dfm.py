...
def dfm(
    ...
    Res = {
        ...
        "r": r,
        "p": p,
        "loglik": LL,  # Return the history list, NOT the single loglik returned by EMstep
    }
    # Display output
...
def InitCond(x, r, p, blocks, optNaN, Rcon, q, nQ, i_idio):
    ...
    for i in range(n_b):  # Loop for each block
        ...
        d, v = eig(
            np.cov(res[:, idx_iM], rowvar=False)
            if len(idx_iM) > 1
            else np.array([[np.var(res[:, idx_iM])]])
        )

        # When len(idx_iM) == 1, d and v are calculated analytically
        if len(idx_iM) == 1:
            d = np.array([np.var(res[:, idx_iM])])
            v = np.array([[1.0]])
        ...
        A_temp = np.matmul(
            np.matmul(np.linalg.inv(np.matmul(Z.T, Z)), Z.T), z
    ...
    for i in range(n_idio):  # Loop for monthly variables
        ...
        try:
            leadZero = (
                np.max(np.where(np.arange(1, T + 1) == np.cumsum(np.isnan(res_i)))) + 1
            )
        except ValueError:
            leadZero = None
        try:
            endZero = -(
                np.max(
                    np.where(np.arange(1, T + 1) == np.cumsum(np.isnan(res_i[::-1])))[0]
                )
                + 1
            )
        except ValueError:
        ...
        """#Original fails with newer numpy versions [sp]
        BM[i, i] = np.matmul(
            np.matmul(np.linalg.inv(np.matmul(res_i[:-1].T, res_i[:-1])), res_i[:-1].T),
            res_i[1:],
        )
        SM[i, i] = np.cov(res_i[1:] - (res_i[:-1] * BM[i, i]), rowvar=False)
        """
        # 2026 fix [sp]
        # For each series i, estimate AR(1) coefficient
        BM[i, i] = np.linalg.lstsq(res_i[:-1].reshape(-1, 1), res_i[1:], rcond=None)[0][
            0
        ][0]
        SM[i, i] = np.var(res_i[1:] - BM[i, i] * res_i[:-1], ddof=1)
...
def EMstep(y, A, C, Q, R, Z_0, V_0, r, p, R_mat, q, nQ, i_idio, blocks):
    ...
    # Store the single float from the array returned by Kalman filter
    loglik = loglik[0]
    ...
    for i in range(num_blocks):  # Loop for each block: factors are uncorrelated
        ...
        # Fix: use highly stable pseudo-inverse instead of straight divide-by-zero risk
        # on diagonal of EZZ_BB
        A_i[:r_i, :rp] = np.matmul(EZZ_FB[:r_i, :rp], np.linalg.pinv(EZZ_BB[:rp, :rp]))
        # Equation 8: Covariance matrix of residuals of VA
        Q_i[:r_i, :r_i] = (
            EZZ[:r_i, :r_i] - np.matmul(A_i[:r_i, :rp], EZZ_FB[:r_i, :rp].T)
        ) / T
    ...
    A_i = np.matmul(
        EZZ_FB, np.linalg.pinv(EZZ_BB)
    )  # Equation 6 via Pseudo-Inverse to prevent Zero Division
    ...
    for i in range(n_bl):  # Loop through unique loadings (e.g. [1 0 0 0], [1 1 0 0])
        ...
        for t in range(T):
            ...
            denom += np.kron(
                np.matmul(
                    Zsmooth[bl_idxM_ind][:, [t + 1]], Zsmooth[bl_idxM_ind][:, [t + 1]].T
                )
            ...
            # E[y_t*f_t' | \Omega_T]
            nom += np.matmul(
                y[idx_iM][:, [t]], Zsmooth[bl_idxM_ind][:, [t + 1]].T
            ) - np.matmul(
                Wt[:, i_idio_i],
                np.matmul(
        ...
        C_new[np.ix_(idx_iM, bl_idxM_ind)] = vec_C.copy().reshape(
            (n_i, rs), order="F"
        )  # CHECK: RESHAPE NEEDS TO BE VERIFIED
        ...
        for j in idx_iQ:
            ...
            i_idio_jQ = np.arange(
                rp1 + n_idio_M + 5 * (idx_jQ), rp1 + n_idio_M + 5 * (idx_jQ + 1)
            )
            # Place quarterly values in output matrix
            V_0_new[np.ix_(i_idio_jQ, i_idio_jQ)] = Vsmooth[0][
                np.ix_(i_idio_jQ, i_idio_jQ)
            ].copy()
            ...
            for t in range(T):
                ...
                nom -= np.matmul(
                    ...
                    np.matmul(
                        np.matmul(
                            np.array([[1, 2, 3, 2, 1]]), Zsmooth[i_idio_jQ][:, [t + 1]]
                        ),
                        Zsmooth[bl_idxQ_ind][:, [t + 1]].T,
                    )
                    + np.matmul(
    ...
    for t in range(T):
        ...
        R_new += (
            ...
            + np.matmul(
                np.matmul(
                    np.matmul(np.matmul(Wt, C_new), Vsmooth[t + 1][:, :]), C_new.T
                ),
                Wt,
            )
    ...
    RR[i_idio_M] = (
        1e-4  # Ensure non-zero measurement error. See Doz, Giannone, Reichlin (2012) for reference.
    )
    RR[nM:] = 1e-4
...
def em_converged(loglik, previous_loglik, threshold=1e-4, check_decreased=1):
    ...
    if check_decreased == 1:
        if (loglik - previous_loglik) < -1e-3:
            print("******likelihood decreased from {} to {}").format(
                previous_loglik, loglik
            )
            decrease = 1
...
def check_Y(Y):
    # --- sp ---
    print(Y)
    assert not np.any(np.isnan(Y)), "Input Y contains NaN values"
    raise ValueError("Y has nan values")


def runKF(Y, A, C, Q, R, Z_0, V_0, use_numba=True):
    ...
    # check_Y(Y)
    S = SKF(
        Y, A, C, Q, R, Z_0[:, 0] if Z_0.ndim > 1 else Z_0, V_0, use_numba=use_numba
    )  # Kalman filter
...
def run_kalman_filter_loop_python(nobs, m, Y, A, C, Q, R, Zu, Vu):
    Zm = np.zeros((m, nobs))
    Vm = np.zeros((nobs, m, m))
    ZmU = np.zeros((m, nobs + 1))
    VmU = np.zeros((nobs + 1, m, m))
    loglik = 0.0
    ...
    for t in range(nobs):
        ...
        if len(valid_idx) == 0:
            ...
            VC = V @ C_t.T
            iF = np.linalg.inv(C_t @ VC + R_t)
            VCF = VC @ iF
            innov = Y_t - (C_t @ Z).reshape(-1, 1)
            Zu = Z + (VCF @ innov).flatten()
            Vu = V - VCF @ VC.T
            loglik += 0.5 * (np.log(np.linalg.det(iF)) - (innov.T @ iF @ innov)[0, 0])
    ...
    return Zm, Vm, ZmU, VmU, loglik


@jit(nopython=True)
def run_kalman_filter_loop(nobs, m, Y, A, C, Q, R, Zu, Vu):
    Zm = np.zeros((m, nobs))
    Vm = np.zeros((nobs, m, m))
    ZmU = np.zeros((m, nobs + 1))
    VmU = np.zeros((nobs + 1, m, m))
    ...
    for t in range(nobs):
        ...
        Z = A @ Zu
        # Prior covariance matrix of Z
        V = A @ Vu @ A.T + Q
        V = 0.5 * (V + V.T)  # Trick to make symmetric

        # CALCULATING POSTERIOR DISTRIBUTION ----------------------------
        # Find which rows in Y are not NaN at time t
        y_col = Y[:, t]
        valid_idx = np.where(~np.isnan(y_col))[0]
        ...
        if len(valid_idx) == 0:
            ...
            # Extract valid covariance rows/cols
            # R_t = R[valid_idx, :][:, valid_idx] would require boolean indexing which is tricky in numba,
            # instead we build it manually or use advanced indexing if numba supports it.
            R_t = np.zeros((len(valid_idx), len(valid_idx)))
            for i, idx1 in enumerate(valid_idx):
                for j, idx2 in enumerate(valid_idx):
                    R_t[i, j] = R[idx1, idx2]
            ...
            # Matrix of population regression coefficients
            VCF = VC @ iF
        ...
        Zu = Zu.reshape(-1)  # ensure Zu is 1D for storage
        Zm[:, t] = Z
        Vm[t, :, :] = V
        ZmU[:, t + 1] = Zu
        VmU[t + 1, :, :] = Vu
...
def SKF(Y, A, C, Q, R, Z_0, V_0, use_numba=True):
    ...
    S["ZmU"][:] = np.nan
    S["VmU"][:] = np.nan
    Zu = Z_0.copy().flatten()  # Z_0|0 (In below loop, Zu gives Z_t | t)
    Vu = V_0.copy()  # V_0|0 (In below loop, Vu guvse V_t | t)

    # You can specify `USE_NUMBA` boolean depending on your environment strategy
    if use_numba:
        Zm, Vm, ZmU, VmU, loglik = run_kalman_filter_loop(
            nobs, m, Y, A, C, Q, R, Zu, Vu
        )
    else:
        Zm, Vm, ZmU, VmU, loglik = run_kalman_filter_loop_python(
            nobs, m, Y, A, C, Q, R, Zu, Vu
        )
    ...
    S["ZmU"] = ZmU
    S["VmU"] = VmU
    S["loglik"] = np.array([loglik])  # keep copyable format

    # Recreate the last iteration of the Kalman filter here for k_t logic (since Numba jit loop does not output it directly)
    y_col = Y[:, nobs - 1]
    valid_idx = np.where(~np.isnan(y_col))[0]
