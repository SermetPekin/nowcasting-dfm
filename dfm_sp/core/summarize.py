...
def summarize(X, Time, Spec):
    ...
    print(
        "T = {} observations from {} to {}\n".format(
            T,
            pd.Timestamp(Time[0]).strftime("%Y-%m-%d"),  # Convert to pd.Timestamp first
            pd.Timestamp(Time[-1]).strftime("%Y-%m-%d"),
        )
    )
    ...
    base.rename(
        index={
            base.index[i]: base.index[i] + " [{}]".format(Spec.SeriesID[i])
            for i in range(N)
        },
        inplace=True,
    )
...
def data_table_prep(option, x=None, freq_dict=None, Time=None, N=None, Spec=None):
    ...
    if option == 1:
        ...
        return "{} / {}".format(
            pd.Timestamp(first).strftime("%b-%Y"),  # Directly format as "Mon-YYYY"
            pd.Timestamp(last).strftime("%b-%Y"),
        )

    elif option == 2:
        min_ = Time[np.where(x == np.min(x))[0][0]]
        max_ = Time[np.where(x == np.max(x))[0][0]]
        import pandas as pd

        return "{} / {}".format(
            pd.Timestamp(min_).strftime("%b-%Y"),  # Directly format as "Mon-YYYY"
            pd.Timestamp(max_).strftime("%b-%Y"),
        )
    elif option == 3:
        return [freq_dict[i] for i in Spec.Frequency]
    elif option == 4:
        return [
            unitTransformed(Spec.Units[i], Spec.Transformation[i], Spec.Frequency[i])
            for i in range(N)
        ]
    else:
